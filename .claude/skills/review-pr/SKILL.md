...
Task ツールを使用して、以下の5つのサブエージェントを **同時に（1つのメッセージで）** 起動してください。
...
全サブエージェントの結果を受け取ったら、以下のフォーマットで統合レポートを作成し、PR コメントとして投稿してください。
...
統合レポートを以下のコマンドで PR コメントとして投稿してください：

```bash
gh pr comment <PR番号> --body "<統合レポートの Markdown>"
```

投稿が成功したら、コメント URL をユーザーに表示してください。
エラーが発生した場合は、エラー内容をユーザーに伝え、レポートをターミナルに出力してください。
